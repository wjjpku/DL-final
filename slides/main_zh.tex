...
\begin{itemize}
  ...
  \item \hl{受控 AdamW 模拟也复现 $\tau\propto1/\eta$($p{=}1.01$)};同模拟里 \hl{$\beta_2$ 消融 $\tau$
  持平(CV 2\%)} $\Rightarrow$ $\beta_2$ 不给 floor 滞后加通道(噪声主导,$v\!\approx\!s^2$ 不变);
...
"训练前预测"的真实用途。
\vfill
\textbf{与 $\gamma$ 的关系。} MPL 的 $\eta_k^{-\gamma}$ 是对这个非绝热滞后的\hl{隐式、低容量近似}:
这解释了为什么 $\gamma$ 必不可少($3\text{--}4\times$)却无法从绝热谱理论推出,也解释了为什么数据多时
MPL 能靠 $\gamma$ 吸收残差、few-shot 时不能。
\end{frame}
